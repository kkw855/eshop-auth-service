...
import com.eshop.auth.domain.user.NewUserInfo

object validators {
  
  sealed trait ValidationFailure(val errorMessage: String)
  private final case class EmptyField(fieldName: String) extends ValidationFailure(s"'$fieldName' is empty")
  ...
  given newUserInfoValidator: Validator[NewUserInfo] = newUserInfo => {
    ...
    val validUserEmail = validateRequired(newUserInfo.email, "email")(_.nonEmpty).andThen(e => validateEmail(e, "email"))
